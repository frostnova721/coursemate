...
router.get("/optout", async (req: Request, res: Response) => {
  ...
  const result = await db.course.deleteCourseEntry({ id: parseInt(userId), courseId: parseInt(course) });

  if(!result) return res.status(404).json({ msg: "user not enrolled in provided course" });
